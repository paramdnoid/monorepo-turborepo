...
| **E-02** | Projekt-Hub, Belege, 7-Tage-Termine, Zeiterfassung, OP, Mahnungsbezug | [`apps/web/app/web/projects/[projectId]/page.tsx`](../apps/web/app/web/projects/[projectId]/page.tsx), Hub-Logik in [`project-hub-content.tsx`](../apps/web/components/web/projects/project-hub-content.tsx); Scheduling-Filter `dateFrom`/`dateTo` in [`apps/api/src/routes/scheduling.ts`](../apps/api/src/routes/scheduling.ts)                                                                                                                                          | **`GET /v1/projects/:id/hub`:** nicht vorhanden (nur parallele Calls / `GET /v1/projects/:id`) — mit Roadmap („optional“) konsistent.    |
...
| **E-09 XRechnung/ZUGFeRD**                                       | offen                       | Keine Treffer `xrechnung`/`zugferd` in `*.ts`/`*.tsx` — **konsistent**.                                                                                                                                                                                                  |
...
| **E-06 Mail-Produktivität**                                      | Spike ja, Queue/Retry offen | API-Route `sales.ts`: keine SMTP/Queue; **BFF** [`email-spike/route.ts`](../apps/web/app/api/web/sales/invoices/[id]/reminders/[reminderId]/email-spike/route.ts) nutzt `createSmtpTransport` / Dry-Run — **Spike bestätigt, produktiver Queue-Flow nicht nachweisbar**. |
...
2. **Explizit optionale** Features (`GET …/hub`) sind im Code **absent** — erwartungskonform, nicht als Lücke zwischen Docs und Code zu werten.
...
- Doc-Update nur falls sich Produktprioritäten ändern; technischer Stand **2026-04-07** passt zur Roadmap vom **2026-04-06**.
