...
| **E-07** | teilweise umgesetzt | Positions-USt (`tax_rate_bps`), Positionsrabatt, Rechnungsarten (`billing_type`), Eltern-/Gutschrift-Referenzen, serverseitige Ketten-Validierung, Projekt-Hub **Teilrechnungsketten**/Saldo; optional Kopf-Rabatt (`header_discount_bps`); Exporte nutzen finalisierten Snapshot inkl. Steueraufschlüsselung | Matrix E-07 |
...
| ---------------------------------------------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **E-07 Belegtiefe** (Steuer split, Rabatte, Teilrechnungsketten) | teilweise (MVP ausgebaut)     | **Aktualisiert:** `sales_invoice_lines.tax_rate_bps` / `discount_bps`; `sales_invoices.billing_type`, `parent_invoice_id`, `credit_for_invoice_id`, optional `header_discount_bps`; API `computeInvoiceTaxBreakdown`, Validierung Referenzen/Projekt/Kunde; Hub `invoiceBillingChains`. Vertiefung (z. B. komplexes Skonto, Teilrechnungs-Soll) bleibt Backlog.                                                                                                     |
