...
type RouteContext = { params: Promise<{ id: string; vacationId: string }> };

export async function PATCH(request: Request, context: RouteContext) {
  const { id, vacationId } = await context.params;
  ...
  try {
    const res = await fetch(
      `${API_BASE}/v1/employees/${encodeURIComponent(id)}/vacation/${encodeURIComponent(vacationId)}`,
      {
        method: "PATCH",
        headers: {
