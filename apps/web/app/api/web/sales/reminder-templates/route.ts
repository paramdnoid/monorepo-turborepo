...
export async function GET(request: Request) {
  ...
  const lang = qp.get("locale") ?? "de";

  try {
    const res = await fetch(
      `${API_BASE}/v1/sales/reminder-templates?locale=${encodeURIComponent(lang)}`,
      {
...
export async function PUT(request: Request) {
  ...
  try {
    const res = await fetch(`${API_BASE}/v1/sales/reminder-templates`, {
      method: "PUT",
      headers: {
