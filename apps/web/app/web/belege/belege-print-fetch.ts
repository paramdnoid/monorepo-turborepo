...
export type BelegeMeContext = MeResponse;

export type BelegeQuotePrintFetchResult =
  | { ok: true; quote: z.infer<typeof salesQuoteDetailResponseSchema>["quote"]; me: BelegeMeContext }
  | { ok: false; status: number };

export type BelegeInvoicePrintFetchResult =
  | { ok: true; invoice: z.infer<typeof salesInvoiceDetailResponseSchema>["invoice"]; me: BelegeMeContext }
  | { ok: false; status: number };

async function fetchMe(token: string): Promise<BelegeMeContext | null> {
...
export async function fetchQuoteForPrint(
  token: string,
  quoteId: string,
): Promise<BelegeQuotePrintFetchResult> {
...
export async function fetchInvoiceForPrint(
  token: string,
  invoiceId: string,
): Promise<BelegeInvoicePrintFetchResult> {
  const [me, salesRes] = await Promise.all([
    fetchMe(token),
