...
type BelegeModuleContentProps = {
  locale: Locale;
};

export function BelegeModuleContent({ locale }: BelegeModuleContentProps) {
  const title = getBelegePageTitle("overview", locale);
  const { noteTitle, noteBody } = getBelegePlaceholderCopy(locale);
      ...
      <Card className="border bg-card shadow-sm">
        ...
        <CardFooter className="flex flex-wrap gap-2 border-t pt-4">
          <Button variant="secondary" size="sm" asChild>
            <Link href="/web/belege/angebote">
              {locale === "en" ? "Quotes" : "Angebote"}
            </Link>
          </Button>
          <Button variant="secondary" size="sm" asChild>
            <Link href="/web/belege/rechnungen">
