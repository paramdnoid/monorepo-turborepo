...
import { fetchQuoteForPrint } from "../../../belege-print-fetch";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  await params;
  return {
    title: `${getBelegePageTitle("quotes", locale)} — ${locale === "en" ? "Print" : "Druck"}`,
    description: getBelegePageDescription("quotes", locale),
  };
}

export default async function BelegeAngebotDruckPage({ params }: PageProps) {
  const { id } = await params;
  const session = await validateWebAccessTokenSession();
  const h = await headers();
  const pathname =
    h.get("x-pathname") ?? `/web/belege/angebote/${encodeURIComponent(id)}/druck`;
  ...
  const backList = "/web/belege/angebote";
  const backDoc = `/web/belege/angebote/${encodeURIComponent(id)}`;
  ...
  return (
    <div className="w-full min-w-0">
      ...
      <BelegeDocumentPrint locale={locale} mode="quotes" quote={result.quote} me={result.me} />
    </div>
  );
}
