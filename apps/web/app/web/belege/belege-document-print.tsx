...
import { getBelegePrintCopy, getBelegeSalesLinesCopy, getBelegeSalesTableCopy } from "@/content/belege-module";
import type { Locale } from "@/lib/i18n/locale";
import { formatMinorCurrency } from "@/lib/money-format";

import { formatBelegeDocumentDate } from "./belege-document-format";
import type { BelegeMeContext } from "./belege-print-fetch";
...
type BelegeDocumentPrintProps =
  | { locale: Locale; mode: "quotes"; quote: QuoteLike; me: BelegeMeContext }
  | { locale: Locale; mode: "invoices"; invoice: InvoiceLike; me: BelegeMeContext };
...
export function BelegeDocumentPrint(props: BelegeDocumentPrintProps) {
  const { locale, me } = props;
  const printCopy = getBelegePrintCopy(locale);
  const tableCopy = getBelegeSalesTableCopy(locale);
  const linesCopy = getBelegeSalesLinesCopy(locale);
  ...
  return (
    <article className="beleg-druck-dokument mx-auto max-w-[210mm] bg-background px-6 py-8 text-foreground shadow-sm print:shadow-none print:max-w-none print:px-0 print:py-0">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          {isQuote ? (
            <MetaRow
              ...
              value={formatBelegeDocumentDate(props.quote.validUntil, locale)}
            />
          ) : (
            <>
              <MetaRow
                label={tableCopy.issued}
                value={formatBelegeDocumentDate(props.invoice.issuedAt, locale)}
              />
              <MetaRow
                label={tableCopy.dueDate}
                value={formatBelegeDocumentDate(props.invoice.dueAt, locale)}
              />
              <MetaRow
                label={tableCopy.paidAt}
                value={formatBelegeDocumentDate(props.invoice.paidAt, locale)}
              />
          ...
          <MetaRow
            ...
            value={formatBelegeDocumentDate(head.updatedAt, locale)}
          />
        </div>
      </section>
      ...
      <section className="mt-10">
        ...
        {sortedLines.length === 0 ? (
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="beleg-druck-tabelle w-full border-collapse text-sm">
              <thead>
