...
import { fetchInvoiceForPrint } from "@/components/web/sales/sales-print-fetch";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  await params;
  return {
    title: `${getSalesPageTitle("invoices", locale)} — ${locale === "en" ? "Print" : "Druck"}`,
...
export default async function SalesInvoicePrintPage({ params }: PageProps) {
  ...
  const pathname =
    h.get("x-pathname") ?? `/web/sales/invoices/${encodeURIComponent(id)}/print`;
  ...
  const backList = "/web/sales/invoices";
