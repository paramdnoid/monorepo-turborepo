...
type KundenCustomerDetailContentProps = {
  locale: Locale;
  customerId: string;
};

export function KundenCustomerDetailContent({
  locale,
  customerId,
}: KundenCustomerDetailContentProps) {
    ...
    return (
      <div className="space-y-4">
        <Button variant="outline" asChild>
          <Link href="/web/kunden">{copy.back}</Link>
        </Button>
            ...
            <form onSubmit={addAddress} className="grid gap-3">
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  maxLength={2}
                  className="uppercase max-w-[5rem]"
                />
              </div>
    ...
    <Card>
      <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 space-y-0">
        <CardTitle className="text-base">
          ...
          {address.isDefault ? ` · ${copy.defaultForKind}` : ""}
        </CardTitle>
