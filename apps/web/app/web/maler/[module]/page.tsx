...
type MalerModulePageProps = {
  params: Promise<{ module: string }>;
};

export function generateStaticParams() {
  return getAllMalerModuleSegments().map((module) => ({ module }));
}

export async function generateMetadata({
  params,
}: MalerModulePageProps): Promise<Metadata> {
  const { module: segment } = await params;
  const locale = await getServerLocale();
  const entry = getMalerModuleBySegment(locale, segment);
  if (!entry) {
    return { title: "Module" };
  }
  return {
...
export default async function MalerModulePage({ params }: MalerModulePageProps) {
  const { module: segment } = await params;
  const locale = await getServerLocale();
  const entry = getMalerModuleBySegment(locale, segment);
  if (!entry) {
