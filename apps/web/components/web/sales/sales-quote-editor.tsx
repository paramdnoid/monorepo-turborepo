...
import { fetchSalesProjectOptions } from "./sales-lookups";
import {
  dateInputToIsoNoon,
  formatMinorCurrency,
  isoToDateInputValue,
...
export function SalesQuoteCreateDialog({
  locale,
  open,
  onOpenChange,
  onCreated,
}: SalesQuoteCreateDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setMasterCustomerId("");
      setError(null);
      void fetchSalesProjectOptions().then(setProjectOptions);
      void fetchSalesCustomerOptions().then(setCustomerStammOptions);
    }
  }, [open]);
...
export function SalesQuoteEditForm({
  locale,
  quote,
  onSaved,
  onCancel,
}: SalesQuoteEditFormProps) {
  const fc = getSalesFormCopy(locale);
  const tc = getSalesTableCopy(locale);
